...
点灯色は時間とともに色相がゆっくり動きます。**明度は段ごとに固定**で、動かすのは色相だけです。CIE Lab では輝度が `L*` だけで決まるため、色相回転では点灯と未点灯の輝度比が数学的に変わらず、段の順序も崩れません。

statuslineの再描画は `refreshInterval` の最小値である**毎秒1回が上限**です。この制約から次のように決めています。

| 項目 | 値 | 理由 |
| --- | --- | --- |
| 1周期 | 14秒 | 1フレームあたりの色差を 4.4 に収めるため |
| 色相の振れ幅 | ±14度 | これ以上動かすと1秒ごとの変化が大きくなりちらつく |
| 段ごとの位相差 | なし（全段同時） | 段ごとにずらすと瞬間の色相幅が16度を超え、「5段が別々の色」に見えてしまう |

全段が同じ位相で動くため、ゲージを流れる波ではなく、**ゲージ全体の色味がゆっくり移り変わる**動きになります。毎秒1回という上限では、段を移動する波は1段あたり数秒かかり、なめらかには見えません。

色は実行時に色空間変換をせず、事前に計算した14位相ぶんの表から引きます。全位相を試験で検証できるようにするためでもあります。

アニメーションには設定側で `refreshInterval` の指定が必要です。指定しない場合、statuslineはイベント発生時にしか再描画されないため、色は不規則にしか変わりません。
...
> [!NOTE]
> `refreshInterval` を指定すると、セッションが待機中でも毎秒プロセスが起動します。1回あたり約14ミリ秒（`git diff` 呼び出しを含む）なので、常時およそ1.3パーセントのCPUを使い続けることになります。大きなリポジトリでは `git diff` がさらに時間を要します。アニメーションが不要なら、この行を削除してください。
...
- Effort アニメーション: 全位相で各段の輝度が 1.05 倍以内、瞬間の色相幅 12 度以内、1フレームの色差 2.0 〜 5.0 に収まることを確認
- セッションコスト: 値がない場合・数値でない場合・負値の場合にセグメントを省略することを確認
- `cargo test --release`: 12件成功。旧パレットへ戻すと色差 0.0 を検出して失敗することも確認
