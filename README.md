...
Powerlineの区切り `` は、**前のセグメントの背景色**で描かれます。したがって隣接する背景色が近いと、区切りそのものが見えなくなります。
...
それでも近くなる隣接（`ユーザー名@ホスト名` → 作業ディレクトリなど）が残るため、背景色どうしのコントラスト比が 1.6 未満の場合は、実線の区切りの代わりに**細い区切り `` を文字色で描きます**。これはPowerlineが同色セグメントに使う標準的な記法で、どの配色でも境界が必ず見えます。

| | 境界のコントラスト比 |
| --- | --- |
| 変更前（単一パレット共有） | 1.00 〜（5組が完全に同色） |
| 変更後（明暗交互） | 2.65 〜 4.16 |
...
- セグメント境界: 使用率36パターンで、背景が近接する箇所と細区切りの出力箇所が完全に一致することを確認。ゲージ列の境界コントラスト比 2.65 以上（変更前は 1.00）
