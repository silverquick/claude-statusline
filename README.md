...
Effort は割合ではなく `minimal` / `low` / `medium` / `high` / `xhigh` の5段階です。連続値ではないので使用率バーとは描き分け、**5つの台形を斜めの境界でつないだゲージ**として表示します。
...
`--` や未知の値のときは、段を決められないためゲージを出さず段階名だけを表示します。
...
- Effort ゲージ: 5段階すべてで段数5・点灯数一致・区切り6個を確認。未知の値ではゲージを出さないことも確認
- `cargo test --release`: 8件成功。旧パレットへ戻すと色差 0.0 を検出して失敗することも確認
