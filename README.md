...
- 端末幅: 入力の `columns`（行の折り返しとサマリ行の切り詰めに使用）。Claude Code v2.1.251 はこのフィールドを渡さないため、実際には次の「行の折り返し」の順序で OS へ端末サイズを問い合わせ、それも取れなければ環境変数 `COLUMNS`、最後に80桁へフォールバックします
...
1. 入力JSONの `columns`
2. OS への端末サイズ問い合わせ（Windows は `CONOUT$` を開いて `GetConsoleScreenBufferInfo`、Unix は制御端末への `ioctl(TIOCGWINSZ)`）
3. 環境変数 `COLUMNS`
4. 上のいずれも取れなければ80桁

`COLUMNS` を OS 問い合わせより後ろへ回しているのは、実際の端末より大きい古い値をエクスポートしてくる端末があるためです（`COLUMNS=180` なのに実コンソールは120桁、という例を確認しています）。Claude Code v2.1.251 は幅を示すフィールドを入力に含めないため、`COLUMNS` だけを信じると折り返し幅が実幅を超え、端末側で行が切り詰められていました。OS への問い合わせは手書きの小さな FFI（Windows は `kernel32` の数関数、Unix は `ioctl` / `open` / `close`）だけで行い、依存クレートは増やしていません。
...
最初の引数が `--subagent` の場合、端末幅と `tasks` 配列を処理します。端末幅は通常モードと同じ順序（入力の `columns` → OS への端末サイズ問い合わせ → 環境変数 `COLUMNS`）で解決し、どれも取れなければ60桁を予算の既定値とします。表示可能な各タスクについて、次の形式を1行ずつ出力します。
...
当初は「端末への ioctl は依存クレートを増やすため使わない」という方針で、入力の `columns` と環境変数 `COLUMNS` だけを見ていました。この判断は撤回しました。Claude Code v2.1.251 が `columns` を渡さないうえ、`COLUMNS=180` をエクスポートしたまま実コンソールは120桁、という端末があり、折り返し幅が実幅を超えて行が右端で切れる不具合を起こしていたためです。

現在は OS へ直接問い合わせます。Windows は `CONOUT$` を開いて `GetConsoleScreenBufferInfo` から可視ウィンドウの幅を取り、Unix は fd 2 / 1 / 0 と `/dev/tty` の順に `ioctl(TIOCGWINSZ)` を試します。標準出力が Claude Code にパイプで捕まえられていても読める経路を選んでいます。実装は `kernel32` の3関数と `ioctl` / `open` / `close` を手書きで宣言した小さな FFI だけで、パニックしうる箇所は持たず、取れなければ `None` を返して次の候補へ進みます。依存クレートは増えていません。
...
- `cargo test --release`: 63件成功。旧パレットへ戻すと色差 0.0 を検出して失敗することも確認
...
- 端末幅の解決: `columns` 未指定でも OS 問い合わせが実幅を返し、`COLUMNS=180` が設定されていても実120桁の端末で各行が120桁以内へ収まること（修正前は約190桁の1行が右端で切れていた）。問い合わせ関数はパニックせず、端末が無ければ `None` を返すことを確認
